...
namespace UCI {
    ...
    namespace Helper {
        ...
        void go_info(Info& info, const std::vector<std::string>& tokens) {
            ...
            info.milliseconds = std::max<uint32_t>(Helper::calculate_tts(time_remaining, inc), 100);
        }
    }
    ...
    void uci_protocol() {
        ...
        while (std::getline(std::cin, cmd)) {
            std::vector<std::string> tokens = Helper::split(cmd);
            ...
            Command keyword = Helper::find_keyword(init_token);
